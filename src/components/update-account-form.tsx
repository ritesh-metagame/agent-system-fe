...
export default function UpdateAccountFormWithCommissionPeriod() {
  ...
  const { user: loggedInUser, role } = useSelector(
  ...
  const [isCommissionFieldDisabled, setIsCommissionFieldDisabled] =
    useState(true);
      ...
      try {
        ...
        if (data.code === "1014" && data.data?.user) {
          ...
          const ownCommissions = {
            eGames: (
              originalCommissionsData.eGames -
              parseFloat(commissionsByCategory.eGames || "0")
            ).toString(),
            sportsBetting: (
              originalCommissionsData.sportsBetting -
              parseFloat(commissionsByCategory.sportsBetting || "0")
            ).toString(),
            specialtyGamesRng: (
              originalCommissionsData.specialtyGamesRng -
              parseFloat(commissionsByCategory.specialtyGamesRng || "0")
            ).toString(),
            specialtyGamesTote: (
              originalCommissionsData.specialtyGamesTote -
              parseFloat(commissionsByCategory.specialtyGamesTote || "0")
            ).toString(),
          };
      ...
      try {
        ...
        if (data.code === "1014" && data.data?.user) {
          ...
          const ownCommissions = {
            ...
            specialtyGamesRng:
              originalCommissionsData.specialtyGamesRng.toString(),
            specialtyGamesTote:
              originalCommissionsData.specialtyGamesTote.toString(),
          };
          ...
          form.setValue("eGamesOwnCommission", ownCommissions.eGames);
          form.setValue(
  ...
  async function handleSubmit(values: z.infer<typeof createAccountFormSchema>) {
    ...
    const specialtyGamesRngCommissionValue = parseFloat(
      values.specialityGamesRngCommission || "0"
    );
    const specialtyGamesToteCommissionValue = parseFloat(
      values.specialityGamesToteCommission || "0"
    );

    console.log({
      eGamesCommissionValue,
      original: originalCommissions.eGames,
    });
  ...
  console.log(isCommissionFieldDisabled && role !== UserRole.SUPER_ADMIN);
      ...
      <CardContent className="px-3 sm:px-6">
        <Form {...form}>
          ...
          >
                    ...
                    <div className="flex flex-col sm:flex-row gap-2">
                      <FormField
                        ...
                        render={({ field: prefixField }) => (
                          ...
                          >
                            <SelectTrigger className="w-full sm:w-[90px] md:w-24">
            ...
            <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 sm:justify-end pt-4 md:pt-6">
              <Button
                variant="secondary"
              ...
              <Button
                variant="default"
                type="submit"
                className="w-full sm:w-auto bg-blue-500 text-white px-8"
